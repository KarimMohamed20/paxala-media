...
| **Plan completion: XSS, correctness, config hardening, RTL** (SEC-06, CORR-05/08, DEP-04/05/06/11, RTL-02) | 🟡 Medium | **SEC-06:** sanitize tiptap HTML (`isomorphic-dompurify` + `sanitizeHtml`) in blog/portfolio/portal detail pages. **CORR-05:** atomic milestone reorder (transaction + full-set check). **CORR-08:** validate PARTIAL payment amount (no NaN to Prisma). **DEP-05:** restrict `next/image` hosts (was `**`). **DEP-06:** body limit 1GB→550MB. **DEP-04:** seed passwords from `SEED_*` env. **DEP-11:** docker-compose requires `POSTGRES_PASSWORD`. **RTL-02:** mobile nav drawer now mirrors (logical CSS + `isRTL`-aware slide). | `src/lib/sanitize.ts` (new), 3 detail pages, `src/app/api/milestones/{reorder,[id]/payment}/route.ts`, `next.config.ts`, `prisma/seed.ts`, `docker-compose.yml`, `src/components/layout/navbar.tsx` |

| **SEO structured data + locale dates + health hardening** (marketing §4.1, I18N-07, DEP-09) | 🟡 Medium | **Structured data:** server-rendered **Article** (blog) + **CreativeWork** (portfolio) + **BreadcrumbList** JSON-LD via the detail-page layouts (React `cache` dedupes the fetch with metadata); generic escaped `JsonLd` renderer. **I18N-07:** locale-aware date formatting (`src/lib/format.ts`, Intl-based) applied to public blog dates (Arabic/Hebrew month names + native numerals). **DEP-09:** health endpoint no longer leaks DB error details. | `src/lib/seo.ts`, `src/lib/format.ts` (new), `src/components/seo/json-ld.tsx`, `src/app/{blog,portfolio}/[slug]/layout.tsx`, `src/app/blog/page.tsx`, `src/app/blog/[slug]/page.tsx`, `src/app/api/health/route.ts` |
